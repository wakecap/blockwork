  ...
  if (type === "system") {
    return (
      <div className={`${getContainerStyles()} ${className}`}>
        <div className={`px-4 py-2 rounded-full text-sm ${getBubbleStyles()}`}>{message}</div>
      </div>
    );
  }
      ...
      >
        ...
        {!isOwn && (
          <div className="flex-shrink-0">
            {avatar ? (
              <img src={avatar} alt={sender} className="w-8 h-8 rounded-full object-cover" />
            ) : (
        ...
        <div className={`flex flex-col ${isOwn ? "items-end" : "items-start"}`}>
          {/* Sender Name */}
          {!isOwn && <span className="text-xs text-neutral-500 mb-1 ml-1">{sender}</span>}
          ...
          <div
            className={`flex items-center space-x-2 mt-1 ${isOwn ? "flex-row-reverse" : "flex-row"}`}
          >
            {showTimestamp && (
              <span className="text-xs text-neutral-400">{formatTime(timestamp)}</span>
            )}
            {showStatus && isOwn && <span className="text-xs">{getStatusIcon()}</span>}
          </div>
        </div>
      </div>
    </div>
