...
import { faBell, faCog, faThLarge, faUsers, faBuilding, faPlus, faDatabase, faFileAlt, faUserCog, faSignOutAlt, faArrowLeft, faCaretDown, faBars, faRocket } from "@fortawesome/free-solid-svg-icons";
...
export const TopNavigator = () => {
  ...
  const [currentProject, setCurrentProject] = useState({ name: 'Construction Site Alpha', initial: 'C' });
  ...
  useEffect(() => {
    ...
    function handleClick(e: MouseEvent) {
      ...
      if (target.closest('button') && target.closest('button')?.textContent?.includes(currentProject.name)) {
        return;
      }
  ...
  const filteredProjectsData = useMemo(() => {
    ...
    const filtered = projectsData.map(orgGroup => ({
      ...orgGroup,
      projects: orgGroup.projects.filter(project =>
    ...
    <div ref={navRef} style={{ position: 'relative', width: '100%' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              >
                ...
                >
                  {filteredProjectsData.length === 0 ? (
                    ...
                    <div key={orgGroup.organization} style={{ marginBottom: orgIndex < filteredProjectsData.length - 1 ? 12 : 0 }}>
                    {/* Organization header */}
                    <div style={{
                      fontSize: 11,
                      fontWeight: 500,
                    ...
                    {orgGroup.projects.map((project, projectIndex) => (
                      <button
                        ...
                        style={{
                          ...
                          padding: '8px 12px',
                          border: 'none',
                          background: currentProject.name === project.name ? '#f3f4f6' : 'transparent',
                          borderRadius: 4,
                          cursor: 'pointer',
                          fontSize: 14,
                          color: '#374151',
          ...
          {/* Pinned items */}
          {pinnedItems.length > 0 && (
            <div ref={pinnedContainerRef} style={{ display: 'flex', alignItems: 'center', gap: 2, position: 'relative' }}>
            ...
            {settingsMenuOpen && (
              <div style={{
                ...
                {settingsMenu.map((item, index) => (
                  ...
                  >
                    <div style={{ width: 16, display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: 0 }}>
                      <FontAwesomeIcon icon={item.icon} style={{ fontSize: 12, color: '#6b7280' }} />
                    </div>
                    <span style={{ 
                      whiteSpace: 'nowrap',
                      overflow: 'hidden',
